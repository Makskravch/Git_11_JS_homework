...
// Задача. Розробити клас «Банер» 
{
    ...
    class Banner {
        constructor(imgPathArr, currentImg = 0, url = 'https://www.google.com/', updateTime = 1000) {
            this.imgPathArr = imgPathArr
            this.currentImg = this.imgPathArr[currentImg]
            this.url = url
            this.updateTime = updateTime
        }

        changeImg() {
            let randIndex = Math.floor(Math.random() * this.imgPathArr.length)
            return this.imgPathArr[randIndex]
        }
        ...
        render(containerId) {
            ...
            link.href = this.url
            img.src = this.changeImg.bind(this)()
...
// Задача. Розробити клас «Список новин» за прикладом рубрики новин на сайті ukr.net
{
    ...
    class NewsList {
        ...
        fiveNews() {
            this.fiveCurrentNews = []
            this.fiveCurrentLink = []
            for (let i = 0; i < 5; i++) {
                let randIndex = Math.floor(Math.random() * this.newsList.length)
                this.fiveCurrentNews.push(this.newsList[randIndex])
                this.fiveCurrentLink.push(this.newsLink[randIndex])
            }
        }
        ...
        render(containerId) {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                let link = document.createElement('a')
                link.href = this.fiveCurrentLink[i]
                link.innerText = this.fiveCurrentNews[i]
                li.appendChild(link)
                ul.appendChild(li)
            }
            div.innerText = this.titleCategory
            div.appendChild(ul)
        }

        updateFiveCurrentNews(containerId) {
            setInterval(this.render.bind(this, containerId), this.updateTime)
        }
    }
